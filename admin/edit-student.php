                                    ...
                                    <div class="form-group row">
                                        ...
                                        <div class="col-sm-4">
                                            <select name="class" id="classID" class="form-control" required="true">
                                                ...
                                                <?php
                                                //fetch classes
                                                $sql = "SELECT * FROM classes WHERE name!=:class";
                                                $conn->query($sql);
                                                $conn->bind(":class", $className);
                                                if($conn->rowCount() > 0){
                                                    $result_set = $conn->fetchMultiple();
                                                    foreach ($result_set as $class) {
                                        ...
                                        <div class="col-sm-4">
                                            <select name="section" id="section" class="form-control" required="true">
                                                ...
                                                <?php
                                                //fetch classes
                                                $sql = "SELECT * FROM sections WHERE name!=:section AND class =:classid";
                                                $conn->query($sql);
                                                $conn->bind(":section", $sectionName);
                                                $conn->bind(":classid", $classid);
                                                if($conn->rowCount() > 0){
                                                    $result_set = $conn->fetchMultiple();
                                                    foreach ($result_set as $section) {
                                                        ?> <option value="<?php echo $section->id; ?>"><?php echo $section->name; ?></option><?php
                                                    }
                        ...
                        <div class="card card-info">
                            ...
                            <div>
                                ...
                                if(empty($result->photo)){
                                    ...
                                    ?> <p style="text-align: center;">
                                    <img src="<?php echo $base_url; ?>upload/<?php echo $result->photo; ?>" alt="<?php echo $result->photo; ?>" width="60%">
                                    </p><?php
                                }
                                ?>
                            </div>
    ...
    $(function () {
        $('#classID').on('change', function () {
            ...
            if(classID){
              $.ajax({
                  type: "POST",
                  url: "<?php echo $base_url; ?>fetch_section.php",
...
<script>
    ...
    $('#change-photo').on('submit', function(event){
        ...
        if($('#change-photo').parsley().isValid()){
            $.ajax({
                url: "<?php echo $conn->base_url();?>update_student_photo.php",
                method: "POST",
                data: new FormData(this),
                contentType: false,
                processData: false,
