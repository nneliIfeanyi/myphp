...
?>
    ...
    <head>
        ...
        // get a valid expiration date (avoid empty string comparison that triggers strict mode error)
        $sql = "SELECT * FROM card_expire_date WHERE expire_date IS NOT NULL LIMIT 1";
        $conn->query($sql);
        $result_set = $conn->fetchSingle();
        $card_expire_date = $result_set ? $result_set->expire_date : null;
