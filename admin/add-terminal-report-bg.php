              ...
              toastr['error']('Photo with the same name already exists.');
                ...
                try{
                    ...
                    if($photo_upload){
                        ...
                        $sql = "UPDATE general_settings SET terminal_report_bg_image =:photo WHERE id =:settings_id";
                        $conn->query($sql);
                        $conn->bind(":photo", $photo_name);
                        $conn->bind(":settings_id", $setting_id);
                        if($conn->execute()){
                            echo "<script>
                      toastr['success']('Terminal Report Background Image updated successfully');
